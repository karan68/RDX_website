...
import { getLaptopsByUsage, type Laptop, type Usage } from "../../shared/laptops";
...
export default function Creator() {
  ...
  const randomized = useMemo(() => {
    return [...getLaptopsByUsage("Creator")].sort(() => Math.random() - 0.5);
  }, []);

  const filtered = useMemo(() => {
    return randomized.filter((d) => (!selectedSegment || d.priceSegment === selectedSegment));
  }, [selectedSegment, randomized]);
  ...
  const canPreview = (title: string) => {
    ...
    // If switching to a different video, require 1.5s since last global play
    if (lastGlobal.title && lastGlobal.title !== title && now - lastGlobal.time < 1500) {
      return false;
    }
    ...
    <div className="">
      ...
      <section className="container mx-auto py-10">
        <h2 className="text-center text-2xl font-semibold md:text-3xl">
          The most creative Windows PCs for content creation
        </h2>
        <div className="mt-6 flex flex-col items-center gap-4 text-sm">
          <Dialog>
            ...
            <DialogContent className="max-w-2xl">
              <DialogHeader>
                <DialogTitle>Price Filters</DialogTitle>
              </DialogHeader>
